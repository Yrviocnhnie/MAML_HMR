...
class mpi_inf_3dhp_dataloader(Dataset):
    ...
    def _load_data_set(self):
        ...
        anno_file_path = os.path.join(self.data_folder, 'annot-test.h5')
        with h5py.File(anno_file_path) as fp:
            # total_kp2d = np.array(fp['gt2d'])
            # total_kp3d = np.array(fp['gt3d'])
            total_kp2d = np.array(fp['part_2D'])
            total_kp3d = np.array(fp['part_3D'])
...
if __name__ == '__main__':
    mpi = mpi_inf_3dhp_dataloader('/data/share/3D/MeshTransformer/datasets/mpi_inf/mpi_inf_3dhp', True, [1.1, 2.0], False, 5)
    l = len(mpi)
    for _ in range(l):
        r = mpi.__getitem__(_)
