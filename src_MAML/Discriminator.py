...
from LinearModel_meta import LinearModel
import config
import util
import torch
import numpy as np
...
class ShapeDiscriminator(LinearModel):
    ...
    def forward(self, inputs, params = None):
        return self.fc_blocks(inputs, params=self.get_subdict(params, 'fc_blocks'))

class PoseDiscriminator(MetaModule):
    def __init__(self, channels):
        ...
        self.conv_blocks = MetaSequential()
        l = len(channels)
        for idx in range(l - 2):
            self.conv_blocks.add_module(
                name = 'conv_{}'.format(idx),
                module = MetaConv2d(in_channels = channels[idx], out_channels = channels[idx + 1], kernel_size = 1, stride = 1)
            )

        self.fc_layer = nn.ModuleList()
        for idx in range(23):
            self.fc_layer.append(MetaLinear(in_features = channels[l - 2], out_features = 1))

    # N x 23 x 9
    def forward(self, inputs, params = None):
        batch_size = inputs.shape[0]
        inputs = inputs.transpose(1, 2).unsqueeze(2) # to N x 9 x 1 x 23
        internal_outputs = self.conv_blocks(inputs, params=self.get_subdict(params, 'conv_blocks')) # to N x c x 1 x 23
        o = []
        high_params = self.get_subdict(params, 'fc_layer')
        for idx in range(23):
            o.append(self.fc_layer[idx](internal_outputs[:,:,0,idx], params=self.get_subdict(high_params, f'{idx}')))
...
class Discriminator(MetaModule):
    ...
    '''
    def forward(self, thetas, params = None):
        batch_size = thetas.shape[0]
        cams, poses, shapes = thetas[:, :3], thetas[:, 3:75], thetas[:, 75:]
        
        shape_disc_value = self.shape_discriminator(shapes, params=self.get_subdict(params, 'shape_discriminator'))
        ...
        pose_disc_value, pose_inter_disc_value = self.pose_discriminator(rotate_matrixs, 
                                                                         params=self.get_subdict(params, 'pose_discriminator'))
        
        full_pose_disc_value = self.full_pose_discriminator(pose_inter_disc_value.contiguous().view(batch_size, -1),
                                                            params=self.get_subdict(params, 'full_pose_discriminator'))
