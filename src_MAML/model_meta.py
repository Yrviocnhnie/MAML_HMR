...
class ThetaRegressor(LinearModel):
    ...
    def forward(self, inputs, params = None):
        ...
        for _ in range(self.iterations):
            ...
            theta = theta + self.fc_blocks(total_inputs, params=self.get_subdict(params, 'fc_blocks'))
            thetas.append(theta)
        return thetas


class HMRNetBase(MetaModule):
    def __init__(self):
    ...
    def _create_sub_modules(self):
        ...
        if self.encoder_name == 'resnet50':
            ...
            self.encoder = Resnet_meta.load_Res50Model()
        else:
            assert 0
        '''
    ...
    def forward(self, inputs, params = None):
        ...
        # ==> Used currently:
        if self.encoder_name == 'resnet50':
            feature = self.encoder(inputs, params=self.get_subdict(params, 'encoder'))
            thetas = self.regressor(feature, params=self.get_subdict(params, 'regressor'))
            detail_info = []
            for theta in thetas:
