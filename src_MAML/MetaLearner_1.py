...
class MetaLearner(nn.Module):
    ...
    def _build_model(self):
        ...
        model_path = config.pre_trained_model['generator']
        if os.path.exists(model_path):
            copy_state_dict(generator.state_dict(), torch.load(model_path))
        else:
            print('model {} not exist!'.format(model_path))
            
        model_path = config.pre_trained_model['discriminator']
        if os.path.exists(model_path):
            copy_state_dict(discriminator.state_dict(), torch.load(model_path))
        else:
            print('model {} not exist!'.format(model_path))
    ...
    def forward(self, spt_data, qry_data):
        ...
        for i in range(batch_size):
            ...
            e_fast_weights_net,  e_fast_weights_grad = gradient_update_parameters(self.generator, e_loss, step_size=self.e_base_lr)
            d_fast_weights_net,  d_fast_weights_grad = gradient_update_parameters(self.discriminator, d_loss, step_size=self.d_base_lr)
            ...
            with torch.no_grad():
                qry_imgs = torch.from_numpy(qry_data[7][i]).to(device)
                generator_outputs = self.generator(qry_imgs, params = e_fast_weights_net)
                loss_kp_2d, loss_kp_3d, loss_shape, loss_pose, \
                    e_disc_loss, d_disc_loss, d_disc_real, d_disc_predict = self._calc_loss(generator_outputs, qry_data, i, params = d_fast_weights_net)
                e_loss = loss_kp_2d + loss_kp_3d + loss_shape + loss_pose + e_disc_loss 
                d_loss = d_disc_loss
            ...
            for k in range(1, self.update_step):
                ...
                spt_imgs = torch.from_numpy(spt_data[7][i]).to(device)
                generator_outputs = self.generator(spt_imgs, params = e_fast_weights_net)
                loss_kp_2d, loss_kp_3d, loss_shape, loss_pose, \
                    e_disc_loss, d_disc_loss, d_disc_real, d_disc_predict = self._calc_loss(generator_outputs, spt_data, i, params = d_fast_weights_net)
                    
                e_loss = loss_kp_2d + loss_kp_3d + loss_shape + loss_pose + e_disc_loss 
                d_loss = d_disc_loss
                
                
                e_fast_weights_net,  e_fast_weights_grad = gradient_update_parameters(self.generator, e_loss, updated_params = e_fast_weights_net, params = e_fast_weights_grad, step_size=self.e_base_lr)
                d_fast_weights_net,  d_fast_weights_grad = gradient_update_parameters(self.discriminator, d_loss, updated_params = d_fast_weights_net, params = d_fast_weights_grad, step_size=self.d_base_lr)
                # e_opt_clone.zero_grad()
                # e_loss.backward()
                # e_opt_clone.step()
    ...
    def save_model(self, result):
        ...
        parent_folder = '/home/richie/MAML_HMR/src/trained_model_1'
    ...
    def finetuning(self, spt_data, qry_data):
        ...
        e_loss_list_qry = [0 for _ in range(self.update_step + 1)]
        d_loss_list_qry = [0 for _ in range(self.update_step + 1)]
        loss_kp_2d_list_qry = [0 for _ in range(self.update_step + 1)]
        loss_kp_3d_list_qry = [0 for _ in range(self.update_step + 1)]
        loss_shape_list_qry = [0 for _ in range(self.update_step + 1)]
        loss_spose_list_qry = [0 for _ in range(self.update_step + 1)]
        d_disc_loss_list_qry = [0 for _ in range(self.update_step + 1)]
        for i in range(batch_size):
            ...
            # calculate the updated and making a deep copy of it
            spt_imgs = torch.from_numpy(spt_data[7][i]).to(device)
            generator_outputs = gen_clone(spt_imgs, params = None)
            loss_kp_2d, loss_kp_3d, loss_shape, loss_pose, \
                e_disc_loss, d_disc_loss, d_disc_real, d_disc_predict = self._calc_loss(generator_outputs, spt_data, i, params = None, finetune = True, new_dis = disc_clone)
                # self._calc_loss(generator_outputs, spt_data, i, params = d_fast_weights_net)
            e_loss = loss_kp_2d + loss_kp_3d + loss_shape + loss_pose + e_disc_loss 
            d_loss = d_disc_loss
            
            e_fast_weights_net,  e_fast_weights_grad = gradient_update_parameters(gen_clone, e_loss, step_size=self.e_base_lr)
            d_fast_weights_net,  d_fast_weights_grad = gradient_update_parameters(disc_clone, d_loss, step_size=self.d_base_lr)
            
            # Test on the query set: using the weights before upadte 
            with torch.no_grad():
                qry_imgs = torch.from_numpy(qry_data[7][i]).to(device)
                generator_outputs = gen_clone(qry_imgs, params = OrderedDict(gen_clone.meta_named_parameters()))
                loss_kp_2d, loss_kp_3d, loss_shape, loss_pose, \
                    e_disc_loss, d_disc_loss, d_disc_real, d_disc_predict = self._calc_loss(generator_outputs, qry_data, i, params = d_fast_weights_net, finetune = True, new_dis = disc_clone)
                ...
                e_loss_list_qry[0] += e_loss
                d_loss_list_qry[0] += d_loss
                loss_kp_2d_list_qry[0] += loss_kp_2d
                loss_kp_3d_list_qry[0] += loss_kp_3d
                loss_shape_list_qry[0] += loss_shape
                loss_spose_list_qry[0] += loss_pose
                d_disc_loss_list_qry[0] += e_disc_loss
            # Test on the query set: using the upadte weighted
            with torch.no_grad():
                qry_imgs = torch.from_numpy(qry_data[7][i]).to(device)
                generator_outputs = gen_clone(qry_imgs, params = e_fast_weights_net)
                loss_kp_2d, loss_kp_3d, loss_shape, loss_pose, \
                    e_disc_loss, d_disc_loss, d_disc_real, d_disc_predict = self._calc_loss(generator_outputs, qry_data, i, params = d_fast_weights_net, finetune = True, new_dis = disc_clone)
                e_loss = loss_kp_2d + loss_kp_3d + loss_shape + loss_pose + e_disc_loss 
                d_loss = d_disc_loss
                ...
                e_loss_list_qry[1] += e_loss
                d_loss_list_qry[1] += d_loss
                loss_kp_2d_list_qry[1] += loss_kp_2d
                loss_kp_3d_list_qry[1] += loss_kp_3d
                loss_shape_list_qry[1] += loss_shape
                loss_spose_list_qry[1] += loss_pose
                d_disc_loss_list_qry[1] += e_disc_loss
                
            for k in range(1, self.update_step):
                # Uptate step k: using updaated weights from step 0
                spt_imgs = torch.from_numpy(spt_data[7][i]).to(device)
                generator_outputs = gen_clone(spt_imgs, params = e_fast_weights_net)
                loss_kp_2d, loss_kp_3d, loss_shape, loss_pose, \
                    e_disc_loss, d_disc_loss, d_disc_real, d_disc_predict = self._calc_loss(generator_outputs, spt_data, i, params = d_fast_weights_net, finetune = True, new_dis = disc_clone)
                    
                e_loss = loss_kp_2d + loss_kp_3d + loss_shape + loss_pose + e_disc_loss 
                d_loss = d_disc_loss
                
                e_fast_weights_net,  e_fast_weights_grad = gradient_update_parameters(gen_clone, e_loss, updated_params = e_fast_weights_net, params = e_fast_weights_grad, step_size=self.e_base_lr)
                d_fast_weights_net,  d_fast_weights_grad = gradient_update_parameters(disc_clone, d_loss, updated_params = d_fast_weights_net, params = d_fast_weights_grad, step_size=self.d_base_lr)
                ...
                e_loss_list_qry[k+1] += e_loss
                d_loss_list_qry[k+1] += d_loss
                loss_kp_2d_list_qry[k+1] += loss_kp_2d
                loss_kp_3d_list_qry[k+1] += loss_kp_3d
                loss_shape_list_qry[k+1] += loss_shape
                loss_spose_list_qry[k+1] += loss_pose
                d_disc_loss_list_qry[k+1] += e_disc_loss
        ...
        mydict = {}
        mydict['imgs'] = qry_imgs
        mydict['thetas'] = generator_outputs[2]
        # generator_outputs[i][0]: theta; shape:[16, 85] 
        # generator_outputs[i][1]: verts; shape:[16, 6890, 3] 
        # generator_outputs[i][2]: j2d; shape:[16, 19, 2] 
        # generator_outputs[i][3]: j3d; shape:[16, 19, 3] 
        # generator_outputs[i][4]: Rs; shape:[16, 24, 3, 3] 
        output = open('./results/finetuned_data.pkl', 'wb')
        pickle.dump(mydict, output)
        output.close()
        
        e_loss = np.array([loss.item() for loss in e_loss_list_qry]) / batch_size
        d_loss = np.array([loss.item() for loss in d_loss_list_qry]) / batch_size
        loss_kp_2d = np.array([loss.item() for loss in loss_kp_2d_list_qry]) / batch_size
        loss_kp_3d = np.array([loss.item() for loss in loss_kp_3d_list_qry]) / batch_size
        loss_shape = np.array([loss.item() for loss in loss_shape_list_qry]) / batch_size
        loss_pose = np.array([loss.item() for loss in loss_spose_list_qry]) / batch_size
        e_disc_loss = np.array([loss.item() for loss in d_disc_loss_list_qry]) / batch_size
    
        return e_loss, d_loss, loss_kp_2d, loss_kp_3d, loss_shape, loss_pose, e_disc_loss
